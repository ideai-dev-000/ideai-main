    ...
    const loadParticles = async () => {
      ...
      const particleConfig = {
        particles: {
          ...
          shape: {
            type: config.animationType === "confetti" ? ["circle", "square"] : "circle",
          },
          opacity: {
            value: 0.6,
  ...
  return (
    <div className="flex flex-col items-center gap-6 relative">
      <div className="relative w-[300px] h-[300px]">
        <div id="particles-container" ref={containerRef} className="absolute inset-0" />
        <div className="absolute inset-0 flex items-center justify-center">{renderSvg()}</div>
      </div>

      <div className="text-center">
        <h2 className="text-3xl font-bold text-foreground">ParticleLogo</h2>
        <p className="text-sm text-muted-foreground mt-2">Powered by tsParticles</p>
      </div>
    </div>
  )
}
