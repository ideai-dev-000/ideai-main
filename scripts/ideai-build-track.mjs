 ...
 * @usage node scripts/ideai-build-track.mjs [parent-app] [build|check|report]
 */
...
function generateReport(parentApp) {
  ...
  if (!buildData) {
    ...
    console.log(`   Run: node scripts/ideai-build-track.mjs ${parentApp} build`);
    return;
  }
...
function main() {
  ...
  if (command === "build") {
    ...
    execSync(`node scripts/ideai-build-check.mjs ${parentApp}`, { stdio: "inherit" });
  } else {
    console.log(`Usage: node scripts/ideai-build-track.mjs [parent-app] [build|check|report]`);
    process.exit(1);
  }
}
