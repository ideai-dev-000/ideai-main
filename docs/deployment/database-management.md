...
**🚀 Quick Start**: Need to edit the database right now? See [Drizzle Studio Quick Reference](./DRIZZLE-STUDIO-QUICK-REF.md)
...
- **URL**: `https://local.drizzle.studio`
  - ⚠️ **Important**: Newer versions of Drizzle Studio (0.31+) use this URL instead of `http://localhost:4983`
  - The URL should work automatically, but if it doesn't resolve, you may need to add to `/etc/hosts`: `127.0.0.1 local.drizzle.studio`
- Browse tables, edit data, run queries visually

**Setup Requirements:**

1. **Environment Variables**: Ensure `DATABASE_URL` is set in `apps/ideai-capabilities/.env.local`:

   ```env
   DATABASE_URL="postgresql://user:password@host:port/database"
   ```

   - The `drizzle.config.ts` automatically loads `.env.local` first, then falls back to `.env`
   - For local development, use your local PostgreSQL connection string
   - For production database access, use the production connection string

2. **Database Connection**: Verify your database is accessible:
   ```bash
   cd apps/ideai-capabilities
   node -e "require('dotenv').config({ path: '.env.local' }); const postgres = require('postgres'); const sql = postgres(process.env.DATABASE_URL); sql\`SELECT version()\`.then(r => { console.log('✅ Connected:', r[0].version); sql.end(); }).catch(e => { console.error('❌ Error:', e.message); sql.end(); });"
   ```
...
- **Error "Unexpected error happened"**: Usually means `DATABASE_URL` is not set correctly in `.env.local`
- **Port already in use**: Kill existing process: `lsof -ti:4983 | xargs kill -9`
- **Can't access URL**: Try adding `127.0.0.1 local.drizzle.studio` to `/etc/hosts`
...
**Problem**: `pnpm db:studio` fails or shows "Unexpected error happened"
...
1. **Check DATABASE_URL in `.env.local`:**
   ...
   - The `drizzle.config.ts` loads `.env.local` first (then `.env` as fallback)

2. **Check port availability:**
   - Default port is 4983
   - Kill process if port in use: `lsof -ti:4983 | xargs kill -9`

3. **Verify database connection:**

   ```bash
   cd apps/ideai-capabilities
   node -e "require('dotenv').config({ path: '.env.local' }); const postgres = require('postgres'); const sql = postgres(process.env.DATABASE_URL); sql\`SELECT version()\`.then(r => { console.log('✅ Connected'); sql.end(); }).catch(e => { console.error('❌ Error:', e.message); sql.end(); });"
   ```

4. **Access URL**:
   - Newer Drizzle Studio versions use: `https://local.drizzle.studio` (not `http://localhost:4983`)
   - If URL doesn't resolve, add to `/etc/hosts`: `127.0.0.1 local.drizzle.studio`

5. **Check drizzle.config.ts:**
   ```bash
   cat apps/ideai-capabilities/drizzle.config.ts
   ```

   - Should load `.env.local` first: `config({ path: ".env.local" }); config();`
...
```bash
# Setup: Ensure DATABASE_URL is in .env.local
cd apps/ideai-capabilities
# Edit .env.local and add:
# DATABASE_URL="postgresql://user:password@host:port/database"

# Open Drizzle Studio (most common use case)
cd apps/ideai-capabilities
pnpm db:studio
...
- **Drizzle Config**: `apps/ideai-capabilities/drizzle.config.ts` (loads `.env.local` automatically)
- **Environment Variables**: `apps/ideai-capabilities/.env.local` (contains `DATABASE_URL` - never commit!)
...
- **[Drizzle Studio Quick Reference](./DRIZZLE-STUDIO-QUICK-REF.md)** - Quick start guide for editing database
